...
const create = async (req, res) => {
    try {
      ...
      if (user) {
        return res.status(200).json({ message: "Usuário já cadastrado", user });
      }
  
      // Cria o usuário se não existir
      const newUser = await User.create({ email, name, token: token });
  
      return res.status(201).json({ message: "User cadastrado com sucesso", user: newUser });
    } catch (err) {
      return res.status(500).json({ error: err.message });
    }
  };
